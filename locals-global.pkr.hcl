locals {
  ...
  # Fallback to the boot disk's storage pool if efi_default_storage_pool is null, or to the default if disks is empty.
  efi_default_storage_pool = var.efi_default_storage_pool != null ? var.efi_default_storage_pool : (length(var.disks) == 0) ? var.disk_default_storage_pool : lookup(var.disks[0], "storage_pool", var.disk_default_storage_pool)
